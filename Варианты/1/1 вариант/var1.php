...
function printsCort ($print1, $print2){
...
foreach ($data as $prints){
	$photo=htmlspecialchars($prints['mainPhoto']['url']);
	?>
	...
	<tr>
		...
		<td width="70%">
			...
			<div>Название: <?=htmlspecialchars($prints['name'])?> </div>
			<div>Тип категории: <?=htmlspecialchars($prints['category']['type'])?> </div>
			<div >Цена:   

				<?php 
				$pricePrint=$prints['prices']['amount'];
             			if (in_array( $pricePrint, $price3)){
                    
					echo "<b class='large_group'>". htmlspecialchars(number_format($prints['prices']['amount'], 2, ',' , ' '  )) . "</b>";
                			echo " ";
				}
				else {
					echo htmlspecialchars(number_format($prints['prices']['amount'], 2, ',' , ' '  ));
					echo " ";
				}
				 ?><?=htmlspecialchars($prints['prices']['curName'] )?>  </div>
			
			<div> <a href=<?=htmlspecialchars ($prints['link'])?>> Ссылка на Яндекс Маркет </a> </div>
		</td>
...
echo "<h2>Средняя цена по каждому производителю:</h2>";
...
for ($i=0;$i<count($data);$i++)
{
	if (array_key_exists($i, $data))
	{
		...
		$newdata1[$i]['amount'] = $data[$i]['prices']['amount'];
	}
};
$newdata1_unic = array(); 
$i = 0; 
...
{ 
	if (!in_array($val[$vendor], $key_array)) 
	{
		...
		$temp_prices = round(array_sum($temp)/count($temp), 2);
		$val[$prices] = $temp_prices;
		$newdata1_unic[$i] = $val;
		$i++;
		unset($temp);
	}
