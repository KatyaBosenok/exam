...
foreach ($data as $cultivators){
	$price[]=$cultivators['prices']['amount'];
}
$description='description';

foreach ($data as $cultivators){
	if ($cultivators['description']=="мотоблок,  двигатель, , бензиновый")
	$motoblok[]=$cultivators['name'];
}
...
foreach ($data as $cultivators){
	...
	<tr>
		...
		<td width="70%">
			...
			<div>Название: <?
				$namePrint=$cultivators['name'];
             			if (in_array( $namePrint, $motoblok)){
                    
					echo "<b class='large_group'>". htmlspecialchars($cultivators['name']) . "</b>";
                			echo " ";
				}
				else {
					echo htmlspecialchars($cultivators['name']);
					echo " ";
				}
			?> </div>
